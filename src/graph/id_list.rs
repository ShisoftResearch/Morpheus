use dovahkiin::types::{OwnedValue, OwnedMap, Value, SharedValue, Map};
...
lazy_static! {
    pub static ref ID_TYPE_LIST: Field = Field::new("*", Type::Map, false, false, Some(vec![
        Field::new(&String::from(ID_TYPES_MAP_KEY), Type::Map, false, true,
            Some(vec![
                Field::new(&String::from(ID_TYPE_SCHEMA_ID_KEY), Type::U32, false, false, None, vec![]),
                Field::new(&String::from(ID_TYPE_ID_LIST_KEY), Type::Id, false, false, None, vec![])
            ]), vec![]) 
    ]), vec![]);
    pub static ref ID_LINKED_LIST: Field = Field::new("*", Type::Map, false, false, Some(vec![
        Field::new(&String::from(NEXT_KEY), Type::Id, false, false, None, vec![]),
        Field::new(&String::from(LIST_KEY), Type::Id, false, true, None, vec![])
    ]), vec![]);
    pub static ref LIST_CAPACITY: usize =
        (MAX_CELL_SIZE as usize - Type::U32.size().unwrap() - Type::Id.size().unwrap()) / Type::Id.size().unwrap();
...
impl<'a> IdList <'a> {
                    ...
                    if let Some(cell) = txn.read(id).await? {
                        if let OwnedValue::Array(ref type_list) = cell.data[*ID_TYPES_MAP_ID] {
                            ...
                            for value in type_list {
                                if let OwnedValue::U32(ref schema_id) = value[*ID_TYPES_SCHEMA_ID_ID] {
                                    res.push(*schema_id);
                                }
                            }
    ...
    async fn get_root_list_id(&mut self, ensure_container: bool) -> Result<Result<Id, IdListError>, TxnError> {
        match self.txn.read_selected(self.container_id, vec![self.field_id]).await? {
            Some(fields) => {
                if let OwnedValue::Id(id) = fields[0] {
                    let type_list_id = {
                        if id.is_unit_id() && ensure_container {
                            let (type_list_id, type_list) = empty_type_list(&self.container_id, self.field_id);
                            let type_list_cell = OwnedCell::new_with_id(TYPE_LIST_SCHEMA_ID, &type_list_id, type_list);
                            self.txn.write(type_list_cell).await?;
                            set_map_by_key_id(self.txn, self.container_id, self.field_id, OwnedValue::Id(type_list_id)).await?;
                            type_list_id
                        } else {id}
                    };
                    if type_list_id.is_unit_id() {
                        ...
                        let mut type_list_cell = if let Some(cell) = self.txn.read(type_list_id).await?
                            { cell } else { return Ok(Err(IdListError::Unexpected)); }; // in this time type list should existed
                        if let OwnedValue::Array(ref type_list) = type_list_cell.data[*ID_TYPES_MAP_ID] {
                        ...
                        if ensure_container {
                            ...
                            let list_cell = OwnedCell::new_with_id(ID_LIST_SCHEMA_ID, &list_id, list_value);
                            self.txn.write(list_cell).await?; // create schema id list

                            let mut id_list_pair_map = OwnedMap::new();
                            id_list_pair_map.insert_key_id(*ID_TYPES_SCHEMA_ID_ID, OwnedValue::U32(self.schema_id));
                            id_list_pair_map.insert_key_id(*ID_TYPES_LIST_ID, OwnedValue::Id(list_id));
                            if let &mut OwnedValue::Array(ref mut type_list) = &mut type_list_cell.data[*ID_TYPES_MAP_ID] {
    ...
    pub async fn add(&mut self, id: &Id) -> Result<Result<(), IdListError>, TxnError> {
        ...
        if match count_cell_list(&mut last_seg) {
            ...
            let next_seg_cell = OwnedCell::new_with_id(ID_LIST_SCHEMA_ID, &next_seg_id, next_seg_value);
            self.txn.write(next_seg_cell.clone()).await?;
            set_map_by_key_id(&mut self.txn, last_seg.id(), *NEXT_KEY_ID, OwnedValue::Id(next_seg_id)).await?;
    ...
    pub async fn remove(&mut self, id: &Id, all: bool) -> Result<Result<(), IdListError>, TxnError> {
        let id_value = OwnedValue::Id(*id);
        let contained_segs = { // collect affected segment cell ids
    ...
    pub async fn clear_segments(&mut self) -> Result<Result<(), IdListError>, TxnError> {
        ...
        let segments: Vec<_> = IdListSegmentIdIterator::new(self.txn, list_root_id).collect().await;
...
impl <'a> IdListSegmentIdIterator<'a> {
    ...
    pub async fn next(&mut self) -> Option<Id> {
        if !self.next.is_unit_id() {
            match self.txn.read_selected(self.next, NEXT_KEY_ID_VEC.clone()).await {
                Ok(Some(fields)) => {
...
impl <'a> IdListIterator <'a> {
    ...
    pub async fn last(self) -> Option<Id> where Self: Sized {
    ...
    pub async fn count(mut self) -> usize where Self: Sized {
