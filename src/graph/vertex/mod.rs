    ...
    V: ToVertexId,
{
    ...
    match txn.read(*id).await? {
        Some(cell) => {
            let remove_field_lists = |id: Id, txn: Transaction, field_id: u64| {
                async move {
                    ...
                    for schema_id in schemas_ids {
                        let mut id_list =
                            IdList::from_txn_and_container(&txn, id, field_id, schema_id);
                        {
                            // remove edge cells
                            let mut iter = match id_list.iter().await? {
                                Ok(iter) => iter,
                                Err(e) => return Ok(Err(RemoveError::IdListError(e))),
                            };
                            while let Some(edge_id) = iter.next().await {
                                let edge = match edge::from_id(
                                    ...
                                    schema_id,
                                    schemas,
                                    iter.segments.id_iter.txn,
                                    edge_id,
                                )
                                .await?
                                {
                                    Ok(edge) => edge,
                                    Err(e) => return Ok(Err(RemoveError::EdgeError(e))),
                                };
                        ...
                        match id_list.clear_segments().await? {
                            // remove segment cells
                            Ok(()) => {}
                            Err(e) => return Ok(Err(RemoveError::IdListError(e))),
                        }
                    }
            ...
            match remove_field_lists(*id, txn.clone(), EdgeDirection::Inbound.as_field()).await? {
                Ok(()) => {}
                Err(e) => return Ok(Err(e)),
            }
            match remove_field_lists(*id, txn.clone(), EdgeDirection::Outbound.as_field()).await? {
                Ok(()) => {}
                Err(e) => return Ok(Err(e)),
            }
...
{
    let id = &vertex.to_id();
    let update_cell = |cell| match update(cell_to_vertex(cell)) {
        Some(vertex) => Some(vertex_to_cell(vertex)),
        None => None,
    };
    let cell = txn.read(*id).await?;
    match cell {
        Some(cell) => match update_cell(cell) {
